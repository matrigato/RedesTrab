...
void ChatRoom :: operator =(const ChatRoom &x){
	...
	/*for (size_t i = 0; i < 20; i++)
	{
		users[i].connectedSocket = x.users[i].connectedSocket;
		strcpy(users[i].userIp, x.users[i].userIp);
		strcpy(users[i].userName, x.users[i].userName);
		users[i].isConnected = x.users[i].isConnected;
	}*/
...
ChatRoom::ChatRoom(const ChatRoom &x){
	for (size_t i = 0; i < 50; i++)
	{
		roomName[i] = x.roomName[i];
	}
